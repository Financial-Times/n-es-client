...
module.exports = async function (url, opts = {}, creds = {}) {
	...
	return getSignedRequest(url, opts, creds);
};
...
async function getSignedRequest(url, opts, creds) {
	creds = creds || {};
	// I need to get domain, region and path
	const { domain, region, path, protocol } = await getUrlData(url);
...
async function getUrlData(url) {
	...
	const domain = await resolveHost(urlObject);
...
async function resolveHost(urlObject) {
